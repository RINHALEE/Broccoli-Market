	...
	<div class="container" style="font-family: 'Hahmlet'">
		<div class="row">
			...
			<%
				...
				ResultSet rs = null;
				String sql = "select * from mail where get_id like '" +sessionId+"'";
			...
			%>
			<tr>
				<th><%= rs.getString("send_id") %></th>
				<th><%= rs.getString("mail_description") %> </th> 
				<td><%= rs.getString("send_time") %> </td>
				<td><a href="./send2.jsp?id=<%=rs.getString("send_id") %>" target="_blank" class="btn btn-light"><strong style="color: #A6A6A6">답장</strong></a>
				<td><a href="./deleteMail2.jsp?id=<%=rs.getString("send_time")%>" class="btn btn-light"><strong style="color: #A6A6A6">삭제</strong></a>
			</tr>
			<% 
				} 
